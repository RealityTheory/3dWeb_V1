gameManagerDeep = function()
{
    let InitGame = function()
    {
        // Lights
        let skyLight = new BABYLON.DirectionalLight("SkyLight", new BABYLON.Vector3(2, -5, 2), global_sceneRef);
        skyLight.position = new BABYLON.Vector3(100, 100, 100);
        //let light1 = new BABYLON.PointLight("Omni", new BABYLON.Vector3(2, -5, -2), global_sceneRef);
        ...
        let lightMinIntensity = [];
        let lightMaxIntensity = [];
        let ceilingObjects = [];
        ...
        let UpdateCeilingState = function(_selection, _isVisible)
        {
            ...
            for(let i = 0; i < ceilingObjects.length; i++)
            {
                ...
                if(ceilingIdentifier == "All" || ceilingIdentifier == _selection.name.split("_")[1])  //add more conditions if each room has it's own ceiling
                {
            ...
            if(newMeshes[i].name.includes(dummyPrefix_Ceiling))
            {
                ...
                if(newMeshes[i].name.includes(dummyPrefix_Camera))
                {
                 ...
                 if(newMeshes[i].name.includes(dummyPrefix_Light))
                 {
                     ...
                     let minLightIntensity = Number(newMeshes[i].name.split("_")[3]);
                     lightMaxIntensity.push(minLightIntensity * 3);
                     lightMinIntensity.push(minLightIntensity);
                     lightTargetIntensity.push(minLightIntensity);
                 }
             ...
             let toggleUIButtons = function(_currentlyPressedIndex)
             {
                 for(let z = 0; z < uiButtonRefs.length; z++)
                 {
                     uiButtonRefs[z].isVisible = !(z === _currentlyPressedIndex);
                 }
             }
             ...
             for(let z = 0; z < uiButtonRefs.length; z++)
             {
                 uiButtonRefs[z].onPointerClickObservable.add(() => {
                     for(let j = 0; j < cameraPoints.length; j++)
                     {
                         ...
                         {
                            /* if(cameraPoints[j].name.split("_")[1] === "Plenary")
                             {
                                 OnPlaneryToggleVideo(true);
                             }*/
                             camManager.SetCameraTarget(cameraPoints[j], false, false);
                 ...
                 for(let q = 0; q < lightRefs.length; q++)
                 {
                     if(lightRefs[q].name.split("_")[1] === uiButtonRefs[z].name)
                     {
                         uiButtonRefs[z].onPointerEnterObservable.add(function() {
                             lightTargetIntensity[q] = lightMaxIntensity[q];
                         });
                         uiButtonRefs[z].onPointerOutObservable.add(function() {
                            lightTargetIntensity[q] = lightMinIntensity[q];
                         });
                     }
                 }
             }
